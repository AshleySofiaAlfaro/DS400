...
X3= whether or not it rained today. (categorical variable)
...
**Interpretation:** Higher humidity levels (\>60) at 3PM seems to increase the chance that it will rain the next day.
...
```{r}
ggplot(weather, aes(day_of_year, fill = raintoday)) +
  geom_histogram(bins = 50)
```

```{r}
ggplot(weather, aes(day_of_year, fill = raintomorrow)) +
  geom_histogram(bins = 50)
```
...
```{r}
qlogis(0.2) # The probability that it will rain. 20%
```
...
rain_model_1 <- stan_glm(raintomorrow ~ humidity9am,
                             ...
                             # -1.4 (20% chance it will rain), 0.7 (Standard Deviation)
...
# cutoff is the probability cutoff to classify a new case as positive (0.5 is the default)
```
...
-   As we lower c, sensitivity (true positives) increases, but specificity (true negatives) decreases.
...
hotel_bookings_selected <- hotel_bookings %>% 
...
```{r}
hotel_bookings_selected %>% 
  count(is_canceled) 
# 0 = no (63.4%), 1 = yes (36.6%)
```

1.  Exploratory Data Analysis: Construct and discuss plots of `is_canceled` vs each of the four potential predictors above

```{r}
ggplot(hotel_bookings_selected, aes(x = lead_time, fill = is_canceled)) +
  geom_histogram()
```

```{r}
ggplot(hotel_bookings_selected, aes(x = is_repeated_guest, fill = is_canceled)) +
  geom_bar()
```

```{r}
ggplot(hotel_bookings_selected, aes(x = average_daily_rate, y = lead_time, color=is_canceled)) +
...
hotel_model <- stan_glm(
  is_canceled ~ lead_time + previous_cancellations + is_repeated_guest,
  data = hotel_bookings_selected, family = binomial,
...
```{r}
classification_summary(model = hotel_model, data = hotel_bookings_selected, cutoff = 0.4)
```
