...
```{r}
chr_data_simplified <- chr_data_simplified %>% 
  drop_na()
```
...
```{r}
health_model <- stan_glm(
  percent_fair_or_poor_health ~ food_environment_index, data = chr_data_simplified,
  family = gaussian,
  prior_intercept = normal(17, 4),
  prior = normal(-1.3, 0.6),
  prior_aux = exponential(0.22222222),
...
food_data_clean <- chr_data_simplified %>%
  filter(!is.na(county)) %>%
  mutate(
    state = tolower(state),
...
map_df <- usa_counties %>%
  left_join(food_data_clean, 
            by = c("region" = "state", "subregion" = "county"))
```

```{r}
us_food_index <- ggplot(map_df, aes(long, lat, group = group, fill = food_environment_index)) +
  geom_polygon(color = "white", linewidth = 0.1) +
  coord_fixed(1.3) +
  scale_fill_viridis_c(option = "plasma", na.value = "gray90") +
  theme_map() +
  labs(
    title = "Food Environment Index Across U.S. Counties",
    fill = "Food\nEnvironment\nIndex"
  )

ggplotly(us_food_index)
```
