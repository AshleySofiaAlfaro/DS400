...
# The posterior is heavily effected by the data (likelihood) since there was no prior belief.
```
...
1.  All prior choices are informative.
    ...
    1.  FALSE - Uncertain priors (i.e., The Clueless) can't be informative.
...
3.  Any prior choice can be overcome by enough data.
    ...
    1.  FALSE - Almost all priors can be overcome by enough data, but if someone is unsure (The Clueless), that can't be overcome by enough data because it is completely effected by it.

4.  The frequentist paradigm is totally objective.

    1.  FALSE - There is subjectivity in all research, which includes frequentist!
...
1.  **Prior: π∼Beta(3,10), data: Y=10 successes in n=13 trials**
    ...
    1.  The posterior is an equal compromise between the data and the prior.
