...
```{r}
ggplot(fake_news, aes(x = type, fill = title_has_excl)) +
  geom_bar()
```

```{r}
ggplot(fake_news, aes(x = type, y = fear, fill = type)) +
  geom_boxplot() +
...
-   The likelihood L(B∣A) is defined as P(A∣B):
    ...
    > Note to self: "given that it is fake" \<- considering articles w/ exclamation point that ARE fake.*
    >
...
```{r}
#Load in synthethic dataset from ChatGPT
set.seed(123)
...
```{r}
likelihood_went_to_beach_on_vacation <- 0.76
```
...
# (prior * likelihood) / probability overall given the situation
```
...
```{r}
pop_vs_soda %>% 
  group_by(word_for_cola) %>% 
  count()
```

```{r}
unique(pop_vs_soda$state)
...
```{r}
ggplot(pop_vs_soda, aes(y = state, fill = word_for_cola)) +
  geom_bar()
```

```{r}
plotly_bar <- ggplot(pop_vs_soda, aes(x = word_for_cola, fill = state)) +
  geom_bar()
...
```{r}
# GOOGLED THE POPULATION % OF REGIONS
priors <- c(
  midwest = 0.21,
  northeast = 0.17,
  south = 0.39,
...
state_summary <- pop_vs_soda %>%
  group_by(state) %>%
  summarise(
    total_responses = n(),
    pop_count = sum(pop, na.rm = TRUE),
    percent_pop = pop_count / total_responses,
    word_for_cola = first(word_for_cola)
  )
...
plot_usmap(data = state_summary, values = "percent_pop", regions = "states", color = "white") +
  scale_fill_continuous(
    low = "lightblue", high = "darkblue",
    name = "% Saying 'Pop'"
  ) +
  theme_minimal() +
  labs(title = "Popularity of 'Pop' Across the U.S.")
```
